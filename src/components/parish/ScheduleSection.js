          ...
          {joursSemaine.map((jour, index) => {
            ...
            return (
              <div key={jour} className={`relative group ${isToday ? 'ring-2 ring-blue-500' : ''}`}>
                <div className="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 p-4 sm:p-6 h-full border-2 border-transparent hover:border-blue-200">
                  {isToday && (
                  ...
                  <div className="text-center mb-4 sm:mb-6">
                    <h3 className={`text-lg sm:text-xl font-bold mb-2 ${isToday ? 'text-blue-600' : 'text-gray-900'}`}>
                      {jour}
                    </h3>
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {hoursForDay.length > 0 ? (
                      hoursForDay.map((horaire) => (
                        <div key={horaire.id} className="flex items-center justify-between bg-gray-50 rounded-2xl p-3 sm:p-4">
                          <div>
                            <div className="text-sm sm:text-base font-semibold text-gray-900">{horaire.heure}</div>
                            <div className="text-xs sm:text-sm text-gray-600">{horaire.type}</div>
                          </div>
